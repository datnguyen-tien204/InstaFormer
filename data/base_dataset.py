...
class BaseDataset(data.Dataset):
    def __init__(self, cfg, train_mode=True):
        ...
        self.dir_A = cfg.dir_A      # source
        self.dir_B = cfg.dir_B      # target
        self.height = cfg.height    # resize height
        self.width = cfg.width      # resize width
        self.transform =transforms.Compose([transforms.ToTensor(),
                                            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
                                            transforms.Resize((self.height, self.width), interpolation=Image.BICUBIC)])
        self.horiz_flip = BoxHorizontalFlip(0.5)

        self.train_mode = train_mode    
        self.use_box = True if cfg.num_box > 0 else False
        self.num_box = cfg.num_box

        print(f'({self.dir_A} -> {self.dir_B}) image translation')
...
class BoxHorizontalFlip(object):
    ...
    def __call__(self, img, bboxes=None):
        ...
        if random.random() < self.p:
            ...
            if use_box:
                # Flip boxes
                cl, x1, y1, x2, y2 = bboxes[:, 0], bboxes[:, 1], bboxes[:, 2], bboxes[:, 3], bboxes[:, 4]
                fx1 = 1 - x2
                fx2 = 1 - x1

                new_boxes = torch.stack((cl, fx1, y1, fx2, y2), axis=-1)
...
# # kyusun edit: return (image, box)
# def get_transform_box(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
#     transform_list = []
#     if grayscale:
#         transform_list.append(transforms.Grayscale(1))
#     if 'fixsize' in opt.preprocess:
